// ref: https://www.habrador.com/tutorials/rope/3-another-simplified-rope/
...
public class rope : MonoBehaviour {
	...
	private int sectionCount = 12;
	private int minSection = 2;
	private int maxSection = 43;
	...
	private void Update() 
	{
		...
		if (Input.GetKeyDown (KeyCode.LeftBracket)) {
			if (sectionCount + 1 <= maxSection){
				sectionCount ++;
				allRopeSections.Add(new RopeSection(allRopeSections[allRopeSections.Count - 1].pos));
			}
		}
		if (Input.GetKeyDown (KeyCode.RightBracket)) {
			if (sectionCount - 1 >= minSection){
				sectionCount --;
